...
const myElementScroll = document.getElementById('myElement');

// Add a scroll event listener to the window
window.addEventListener('scroll', () => {
    if (!myElementScroll) return;
    // Get the element's position relative to the viewport
    const rect = myElementScroll.getBoundingClientRect();
...
const myElementCentered = document.getElementById('myElementId'); // Replace with your element's ID

window.addEventListener('scroll', () => {
    if (!myElementCentered) return;
    if (isElementCentered(myElementCentered)) {
        console.log('Element is centered!');
        // Add or remove a class, trigger an animation, etc.
        myElementCentered.classList.add('centered');
    } else {
        console.log('Element is not centered.');
        myElementCentered.classList.remove('centered');
    }
});

// unknown
const wordsAndPunctuation = typeof paragraph !== 'undefined' ? paragraph.split(/\b(\S+)\b/) : [];
...
// Example usage:
const myParagraph =
    'This is a sample paragraph. It includes punctuation, like commas, periods, and even exclamation marks!';
...
const anotherParagraph =
    'Another example with some special characters like @#$%^&*()_+=-`~[]\\{}|;\':",./<>?';
const anotherCharArray = paragraphToArrayEnhanced(anotherParagraph);
console.log(anotherCharArray);

// words to array
function paragraphToWordsArray(paragraph) {
    return paragraph.split(' ');
}

// Example paragraph
const paragraph =
    "Due to the complexity and security considerations, it's recommended to consult the Google Drive API documentation and use a secure environment for development and deployment. I[...]";
const wordsArray = paragraphToWordsArray(paragraph);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function animate() {
        if (zPosition < maxZPosition) {
            zPosition += speed;
            if (box) box.style.transform = `translateZ(${zPosition}px)`;
...
// sender mail address (example HTML included as string for reference)
const senderMailHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
const autoProductMailHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  ...
  <script>
    function sendEmail() {
      ...
      var mailtoLink =
        "mailto:" + email + "?subject=" + encodeURIComponent(subject) + "&body=" + encodeURIComponent(body.replace(/<br>/g, "\n"));
...
// color samples
const sampleHex = '#00FF7F';
const sampleHsl = 'hsl(239, 60%, 50%)';

// font awesome
const fontAwesomeCdn = 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css';
...
// font and icons references (kept as strings)
const googleFontsUrl = 'https://fonts.google.com/';
const bootstrapIconsUrl =
    'https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css';

// translate (example snippets kept as strings)
const googleTranslateSnippet = `<div id="google_translate_element"></div>
...
// notes
const notes = `imgage > squoosh.app
...
function animate() {
    ...
    if (imageTop < viewportHeight && imageBottom > 0) {
        ...
        const progress = Math.min(
            1,
            (scrollY - imageTop + viewportHeight) / (imageBottom - imageTop + viewportHeight)
        );
